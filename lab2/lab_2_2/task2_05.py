...
def cache(func): # декоратор, принимает функцию, которую нужно обернуть
    cached_results = {} # словарь для хранения результатов — ключ: аргументы, значение: результат

    def wrapper(*args): # будет вызываться вместо оригинальной функции
...
@cache                                    # применяем декоратор — теперь slow_add будет кэшироваться
def slow_add(a, b):
